...
class Filter{
    ...
    protected function run(){
        ...
        if(!$this->groups){
            $this->groups = $this->getGroupsAdmin();
    ...
    protected function getGroups(){
        ...
        return \R::getAssoc("SELECT id, title FROM attribute_group WHERE category_id = '{$url}'");
    }

    protected function getGroupsAdmin(){

        return \R::getAssoc('SELECT id, title FROM attribute_group');
    }

    protected static function getAttrs(){
        $data = \R::getAssoc('SELECT * FROM `attribute_value` ORDER BY value ASC ');
        $attrs = [];
        foreach($data as $k => $v){
