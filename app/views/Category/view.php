...
.lim select {
    ...
    @media (max-width: 767px) {
        ...
        .lim span {display: inline-block;padding-top: 6px}
        .lim select {
            float: right;
        }
        .blockPodCategory {
...
<div class="prdt">
    <div class="container">
        <div class="block-nav-podcategory">
            <?php
            ...
            $url = $url[0];
            $url = basename($url);
            $sql = "SELECT * FROM category WHERE alias = '{$url}'";
            $id;
            ...
            foreach($dbh->query($sql) as $rows) {
                echo '<div class="blockPodCategory"><a href="category/'.$rows['alias'].'">'.$rows['title'].'</a></div>';
            }

            ?>
            <div class="lim">
                <span>Показувати по:</span>
                <select name="limit" class="blockPodCategory">
                    ...
                    <option value="15"<?php echo isset($_GET['limit']) && $_GET['limit'] == 15 ? ' selected' : ''; ?>>15</option>
                </select>
            </div>
        </div>
        <div class="prdt-top row">
            <div class="col-md-9 prdt-left">
                ...
                <?php else: ?>
                    <h3>В цій категорії поки що нема товарів...</h3>
                <?php endif; ?>
            </div>
